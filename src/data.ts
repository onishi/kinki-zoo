import type { Zoo } from "./types";
import { JAZA_KINKI_ZOOS, JAZA_MEMBER_SOURCE_URL } from "./jaza-members";

const curatedZoos: Zoo[] = [
  {
    id: "tennoji-zoo",
    name: "天王寺動物園",
    nameKana: "てんのうじどうぶつえん",
    prefecture: "osaka",
    address: "大阪府大阪市天王寺区茶臼山町1-108",
    lat: 34.6494,
    lon: 135.5063,
    openingHours: "9:30〜17:00（入園は16:00まで）",
    closedDays: "毎週月曜日（祝日の場合は翌平日）、年末年始",
    admission: "大人500円、中学生以下無料",
    website: "https://www.tennojizoo.jp/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E5%A4%A9%E7%8E%8B%E5%AF%BA%E5%8B%95%E7%89%A9%E5%9C%92",
    features: ["アフリカサバンナ", "大型類人猿", "夜行性動物館"],
  },
  {
    id: "ikeda-zoo",
    name: "五月山動物園",
    nameKana: "さつきやまどうぶつえん",
    prefecture: "osaka",
    address: "大阪府池田市綾羽2-5-33",
    lat: 34.8168,
    lon: 135.4316,
    openingHours: "9:00〜17:00（入園は16:30まで）",
    closedDays: "毎週火曜日（祝日の場合は翌平日）、年末年始",
    admission: "無料",
    website: "https://www.satsukiyamazoo.com/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E6%B1%A0%E7%94%B0%E5%B8%82%E7%AB%8B%E3%81%93%E3%81%A9%E3%82%82%E5%8B%95%E7%89%A9%E5%9C%92",
    features: ["ポニー乗馬", "ふれあいコーナー"],
  },
  {
    id: "kyoto-zoo",
    name: "京都市動物園",
    nameKana: "きょうとしどうぶつえん",
    prefecture: "kyoto",
    address: "京都府京都市左京区岡崎法勝寺町岡崎公園内",
    lat: 35.0142,
    lon: 135.7826,
    openingHours: "9:00〜17:00（入園は16:00まで）",
    closedDays: "毎週月曜日（祝日の場合は翌平日）、年末年始",
    admission: "大人750円、中学生以下無料",
    website: "https://zoo.city.kyoto.lg.jp/zoo/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E4%BA%AC%E9%83%BD%E5%B8%82%E5%8B%95%E7%89%A9%E5%9C%92",
    features: ["ゴリラ", "ニシローランドゴリラ", "もぐもぐタイム"],
  },
  {
    id: "kobe-oji-zoo",
    name: "神戸市立王子動物園",
    nameKana: "こうべしりつおうじどうぶつえん",
    prefecture: "hyogo",
    address: "兵庫県神戸市灘区王子町3-1",
    lat: 34.7126,
    lon: 135.2019,
    openingHours: "9:00〜17:00（入園は16:30まで）",
    closedDays: "毎週水曜日（祝日の場合は翌平日）、年末年始",
    admission: "大人600円、中学生以下無料",
    website: "https://www.kobe-ojizoo.jp/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E7%8E%8B%E5%AD%90%E5%8B%95%E7%89%A9%E5%9C%92",
    features: ["ジャイアントパンダ", "コアラ", "ナイトズー（夏季）"],
  },
  {
    id: "kobe-animal-kingdom",
    name: "神戸どうぶつ王国",
    nameKana: "こうべどうぶつおうこく",
    prefecture: "hyogo",
    address: "兵庫県神戸市中央区港島南町7-1-9",
    lat: 34.6547,
    lon: 135.2226,
    openingHours: "10:00〜17:00（営業カレンダー参照）",
    closedDays: "営業カレンダー参照",
    admission: "公式サイト参照",
    website: "https://www.kobe-oukoku.com/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E7%A5%9E%E6%88%B8%E3%81%A9%E3%81%86%E3%81%B6%E3%81%A4%E7%8E%8B%E5%9B%BD",
    features: ["ハシビロコウ", "マヌルネコ", "動物とのふれあい"],
  },
  {
    id: "himeji-zoo",
    name: "姫路市立動物園",
    nameKana: "ひめじしりつどうぶつえん",
    prefecture: "hyogo",
    address: "兵庫県姫路市本町68姫路城内",
    lat: 34.8418,
    lon: 134.6949,
    openingHours: "9:00〜17:00（入園は16:30まで）",
    closedDays: "年末年始",
    admission: "大人210円、中学生以下無料",
    website: "https://www.city.himeji.lg.jp/dobutuen/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E5%A7%AB%E8%B7%AF%E5%B8%82%E7%AB%8B%E5%8B%95%E7%89%A9%E5%9C%92",
    features: ["チンパンジー", "ホワイトライオン", "姫路城隣接"],
  },
  {
    id: "himeji-central-park",
    name: "姫路セントラルパーク",
    nameKana: "ひめじせんとらるぱーく",
    prefecture: "hyogo",
    address: "兵庫県姫路市豊富町神谷1434",
    lat: 34.9227,
    lon: 134.7436,
    openingHours: "営業カレンダー参照",
    closedDays: "営業カレンダー参照",
    admission: "公式サイト参照",
    website: "https://www.central-park.co.jp/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E5%A7%AB%E8%B7%AF%E3%82%BB%E3%83%B3%E3%83%88%E3%83%A9%E3%83%AB%E3%83%91%E3%83%BC%E3%82%AF",
    features: ["ドライブスルーサファリ", "ウォーキングサファリ", "遊園地"],
  },
  {
    id: "awaji-farm-park",
    name: "淡路ファームパーク イングランドの丘",
    nameKana: "あわじふぁーむぱーく いんぐらんどのおか",
    prefecture: "hyogo",
    address: "兵庫県南あわじ市八木養宜上1401",
    lat: 34.2894,
    lon: 134.8206,
    openingHours: "9:30〜17:00（季節により変動）",
    closedDays: "年中無休（悪天候を除く）",
    admission: "大人1,600円、小中学生900円",
    website: "https://www.england-hill.com/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E3%82%A4%E3%83%B3%E3%82%B0%E3%83%A9%E3%83%B3%E3%83%89%E3%81%AE%E4%B8%98",
    features: ["コアラ", "羊の放牧", "淡路島"],
  },
  {
    id: "gokatsura-animal-park",
    name: "ごかつら池どうぶつパーク",
    nameKana: "ごかつらいけどうぶつぱーく",
    prefecture: "mie",
    address: "三重県多気郡多気町五桂992",
    lat: 34.4662,
    lon: 136.5512,
    openingHours: "営業カレンダー参照",
    closedDays: "営業カレンダー参照",
    admission: "公式サイト参照",
    website: "https://gokatsura.jp/",
    features: ["カラカル", "ニホンカモシカ", "動物とのふれあい"],
  },
  {
    id: "adventure-world",
    name: "アドベンチャーワールド",
    nameKana: "あどべんちゃーわーるど",
    prefecture: "wakayama",
    address: "和歌山県西牟婁郡白浜町堅田2399",
    lat: 33.6858,
    lon: 135.3555,
    openingHours: "10:00〜17:00（季節により変動）",
    closedDays: "不定休（公式サイト参照）",
    admission: "大人5,000円、中学生3,800円、小学生3,200円",
    website: "https://www.aws-s.com/",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E3%82%A2%E3%83%89%E3%83%99%E3%83%B3%E3%83%81%E3%83%A3%E3%83%BC%E3%83%AF%E3%83%BC%E3%83%AB%E3%83%89",
    features: ["ジャイアントパンダ", "サファリワールド", "イルカショー"],
  },
  {
    id: "wakayama-castle-zoo",
    name: "和歌山城動物園",
    nameKana: "わかやまじょうどうぶつえん",
    prefecture: "wakayama",
    address: "和歌山県和歌山市一番丁3",
    lat: 34.2302,
    lon: 135.1722,
    openingHours: "9:00〜17:30",
    closedDays: "12月29日〜1月1日",
    admission: "無料",
    website: "https://wakayamajo.jp/animal/index.html",
    wikipediaUrl: "https://ja.wikipedia.org/wiki/%E5%92%8C%E6%AD%8C%E5%B1%B1%E5%9F%8E%E5%8B%95%E7%89%A9%E5%9C%92",
    features: ["白いライオン", "白いトラ", "和歌山城隣接"],
  },
];

function normalizeWebsiteHost(website: string): string {
  return new URL(website).hostname.replace(/^www\./, "");
}

const jazaZooHosts = new Set(
  JAZA_KINKI_ZOOS.map((facility) => normalizeWebsiteHost(facility.website))
);

export const zoos: Zoo[] = curatedZoos.map((zoo) => {
  if (!jazaZooHosts.has(normalizeWebsiteHost(zoo.website))) return zoo;
  return {
    ...zoo,
    directorySourceName: "日本動物園水族館協会（JAZA）加盟園館検索",
    directorySourceUrl: JAZA_MEMBER_SOURCE_URL,
  };
});
